...
class PollStream:
    ...
    def register(self, stream):
        self.poule.register(stream, select.POLLIN | select.POLLERR | select.POLLHUP)
        self.lst.append(stream)

    def unregister(self, stream):
        self.poule.unregister(stream)
        self.lst.remove(stream)
    
    def scrute(self, timeout):
        fd, event = self.poule.poll(timeout)
